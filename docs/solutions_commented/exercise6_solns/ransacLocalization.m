...
% query_keypoints should be 2x1000 (x,y coordinates in image frame)
...
addpath('../../Ex2/exercise2_solns/');

% RANSAC
for i = 1:num_iterations
    % sample data according model specifications (k)
    ...
    if use_p3p
        ...
        % estimate rotation matrix from sampled keypoints and landmarks in
        % this iteration (create current modell guess)
    ...
    % Count inliers in whole image with current rotation matrix guess (test
    % current modell)
    ...
    % Better model found then in previous iteration --> update inlier mask
    % (save guess)
...
if max_num_inliers == 0
    ...
    % refine rotation matrix with all inlier points of image from best
    % modell
